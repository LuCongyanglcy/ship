...
public class Decode300 implements DecodeReceiveData {

	///////申请数据邻令
	byte[] sendCommand={0x24, 0x41, 0x00, 0x62, 0x00, 0x00, 0x01, (byte) 0x8f, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x01, 0x5b, 0x0D, 0x0A, 0xFF-256, 0xFF-256, 0xFF-256, 0xFF-256, 0xFF-256, 0xFF-256, 0xFF-256, 0xFF-256, 0xFF-256, 0xFF-256, 0xFF-256, 0xFF-256, 0xFF-256, 0xFF-256, 0xFF-256, 0xFF-256, 0xFF-256, 0xFF-256, 0xFF-256, 0xFF-256, 0xFF-256, 0xFF-256};
	...
	////数据
	private int aiNum=200;
	private int diNum=40;
		...
		while(receiveLen>0){
			...
			if(nDB==98 && receiveBuf[1]=='A'){
				...
				if(receiveLen>=dataLen){
					...
					if(receiveBuf[dataLen-2]==0x0d && receiveBuf[dataLen-1]==0x0a && beg==0 && end==399 && checkSum(dataLen-4, wCheck)){
						/////db98
						int iSize=40;
						int ibegin=0;
...
//						iSize=23;
//						for(int i=0;i<iSize;i++){
//							plcData.aiData[ibegin+i]=(receiveBuf[ibuffBegin+i*2]<<8 &0xFF00)|(receiveBuf[ibuffBegin+i*2+1] &0xFF);
//						}
						////模量量4字节
						ibuffBegin+=iSize;
						ibegin=0;
